...
<script>
        ...
        var beers = [
            ...
            // 여기에 각 맥주에 대한 객체들을 동적으로 생성하여 추가해주세요.
     ...
     // 초기화 함수
        ...
        function draw_products(price, type) {
            ...
            var count = 0; // 상품 개수를 세기 위한 변수
            // 맥주 데이터 배열을 순회합니다.
            for (var i = 0; i < beers.length; i++) {
                var beer = beers[i]; // 현재 맥주 정보를 가져옵니다.
                // 설정된 가격, 타입, 색상에 따라 표시할 제품인지 확인합니다.
                if ((parseInt(beer.price) <= parseInt(price) || parseInt(price) === 70000) && (beer.type === type || type === "All") && checkColor(beer.color)) {
                    var card = draw_card(beer); // 해당하는 제품 카드를 생성합니다.
                    productContainer.insertAdjacentHTML('beforeend', card); // 제품을 컨테이너에 추가합니다.
                    count++; // 상품 개수 증가
                }
            }
            // 상품 개수를 표시하는 요소에 개수를 업데이트합니다.
            document.getElementById("articlesCount").textContent = count + "개";
        }
...
<section class="content mt-8 text-xl px-4">
	...
	<div class="badge badge-outline" id="articlesCount">0개</div>
